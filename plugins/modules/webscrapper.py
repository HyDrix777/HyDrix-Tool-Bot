...
@Client.on_message((filters.regex("https") | filters.regex("http") | filters.regex("www")) & filters.private)
async def scrapping(bot, message):
    ...
    try:  # Extracting Raw Data From Webpage ( Unstructured format)
        ...
        file_write.close()
        await message.reply_document(f"RawData-{message.chat.username}.txt", caption="©@HTGToolBot", quote=True)
    ...
    try:
        ...
        file_write.close()
        await message.reply_document(f"HtmlData-{message.chat.username}.txt", caption="©@HTGToolBot", quote=True)
