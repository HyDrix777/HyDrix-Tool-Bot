...
__MODULE__ = "Inline"
__HELP__ = """
I'm more efficient when added as group admin. 
By the way these commands can be used by
        ...
        if text.strip() == "":
            ...
            if len(text.split()) < 2:
                return await client.answer_inline_query(
                    query.id,
                    results=answers,
                    switch_pm_text="Logo creator | logo [TEXT]",
                    switch_pm_parameter="inline",
                )
            tex = text.split(None, 1)[1].strip()
            answerss = await logo_func(answers, tex)
            await client.answer_inline_query(
                query.id, results=answerss, cache_time=2
            )
